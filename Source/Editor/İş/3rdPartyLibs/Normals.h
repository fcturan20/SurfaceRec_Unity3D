...
public:
	...
	void loadPositions(const glm::vec3* PositionsPTR, unsigned int PositionCount) {
		std::vector<Eigen::Vector3d> points(PositionCount);
		for (unsigned int PositionIndex = 0; PositionIndex < PositionCount; PositionIndex++) {
			points[PositionIndex] = Eigen::Vector3d(PositionsPTR[PositionIndex].x, PositionsPTR[PositionIndex].y, PositionsPTR[PositionIndex].z);
		}
		pts.resize(points.size(), 3);
		for (unsigned int PositionIndex = 0; PositionIndex < points.size(); PositionIndex++) {
			pts.row(PositionIndex) = points[PositionIndex];
		}
	}
	...
	void getNormalList(std::vector<glm::vec3>& NormalList) {
		NormalList.clear();
		NormalList.resize(nls.rows());
		for (unsigned int NormalIndex = 0; NormalIndex < NormalList.size(); NormalIndex++) {
			NormalList[NormalIndex] = glm::vec3(nls(NormalIndex, 0), nls(NormalIndex, 1), nls(NormalIndex, 2));
		}
	}
